#!/usr/bin/env bash
# smoke.sh — interflow-ip-video 全链路冒烟（3 镜短稿，零 LLM）。
# 覆盖：起项目 → TTS(你自己的火山克隆音+气口+口型包络) → 建帧(episode.smoke.mjs) →
#       FE 五步管线 → gate_redlines(pre) → render → gate_redlines(post, ffprobe)。
# 用途：改 skill / 换机部署 / 容器镜像验证后跑一次，绿了 = 机械链路可用（Waza test.yml 思路）。
# 消耗：一次 3 句短稿的火山 TTS 调用（几分钱）+ 约 2-4 分钟渲染。
set -uo pipefail
SKILL="$(cd "$(dirname "$0")/.." && pwd)"
PROJ="$SKILL/videos/_smoke"
...
fail() { echo ""; echo "✗ SMOKE FAIL @ $1"; exit 1; }
...
echo "== smoke: TTS =="
"$TTSPY" "$SKILL/scripts/tts_clone.py" --script SCRIPT.md --out-dir . --gap-map "3" || fail "TTS"
