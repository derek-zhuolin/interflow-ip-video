...
- [ ] **9 一眼假验收**（双 take 路径出片必跑）：`node <SKILL_DIR>/scripts/review_gate.mjs --project . --render renders/video.mp4` → 脚本判定动效锚点 ≤150ms / 无手写秒数孤儿 / 口型开口对首词 / 无 ≥3s 冻结段 / 语速曲线在真人基准带（无 voice_baseline.json 时语速项自动跳过并留痕）；再看 `qc/pauses/` 停顿截帧逐张确认有可读 hold 画面。全过才交付，不过按 REVIEW_REPORT 模式一次修一个变量。
